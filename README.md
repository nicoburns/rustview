...
```bash
cargo ndk -t arm64-v8a -o examples/vello_editor/java/src/main/jniLibs/ build -rp android-view-demo
./gradlew build
...
```bash
cargo ndk -t arm64-v8a -o examples/masonry/java/src/main/jniLibs/ build -rp android-view-masonry-demo
./gradlew build
./gradlew installDebug
